...
router.get('/api/profile/me', requireUser, async (req,res)=>{
  const pf = await getProfileByUserId(req.user.id, req.user.id);
  // doplniť rovnako ako pri /:username
  res.json({ 
    ok:true, 
    profile: { 
      userId: req.user.id,
      username: req.user.username,
      created_at: req.user.created_at,  // TOTO
      ...pf 
    } 
  });
});
...
router.get('/api/profile/:username', async (req,res)=>{
  ...
  res.json({ ok:true, profile: { username: u.username, created_at: u.created_at, userId: u.id, ...pf } });
});
...
// GET /api/profile/id/:id  -> view profile by numeric id
router.get('/api/profile/id/:id', async (req, res) => {
  ...
  const pf = await getProfileByUserId(u.id, viewer);
  // doplníme username/created_at pre frontend
  res.json({ ok:true, profile: { username: u.username, created_at: u.created_at, userId: u.id, ...pf } });
});
